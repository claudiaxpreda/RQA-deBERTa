...
from prepare_dataset_llama import get_input_qa

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
HUGGING_TOKEN = "hf_{}"
API_KEY_WB = "{}"
BASE_MODEL = "meta-llama/Llama-3.2-3B"
FINETUNED_MODEL = "llama32-3b_qa_ft-v2"
LR = 2e-5
...
train_data = get_input_qa('train', HUGGING_TOKEN)
val_data = get_input_qa('validation', HUGGING_TOKEN)
...
trainer.model.push_to_hub('claudiapreda/' + FINETUNED_MODEL, token=HUGGING_TOKEN)
tokenizer.push_to_hub('claudiapreda/' + FINETUNED_MODEL, token=HUGGING_TOKEN)